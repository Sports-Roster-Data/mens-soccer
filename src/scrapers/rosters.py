    ...
    with open(f"/Users/dwillis/code/wbb/ncaa/rosters_{season}.csv", 'w') as output_file:
        ...
        for team in teams_with_urls:
            try:
                ...
                if team['ncaa_id'] in [8]:
                    roster = roster_player_scraper(team, season)
                    ...
                    roster = player_card(team, season)
                ...
                elif team['ncaa_id'] in [127, 326, 762, 8981, 30225]:
...
def roster_player_scraper(team, season):
    ...
    players = []
    player_cards = soup.select('.sidearm-roster-player')
    for card in player_cards:
        # Initialize a dictionary for each player
        player_data = {}

        # Name
        name_tag = card.select_one('.sidearm-roster-player-name a')
        player_data['name'] = name_tag.get_text(strip=True) if name_tag else ''

        # URL
        full_url = urljoin(url, name_tag['href']) if name_tag and 'href' in name_tag.attrs else ''
        player_data['url'] = full_url

        # Extract the ID from the URL, assuming it's the last part of the path
        if full_url:
            player_id = urlparse(full_url).path.split('/')[-1]
            player_data['id'] = player_id
        else:
            player_data['id'] = ''

        # Year
        year_tag = card.select_one('.sidearm-roster-player-academic-year')
        player_data['year'] = year_tag.get_text(strip=True) if year_tag else ''

        # Height
        height_tag = card.select_one('.sidearm-roster-player-height')
        player_data['height'] = height_tag.get_text(strip=True) if height_tag else ''

        # Position
        position_tag = card.select_one('.sidearm-roster-player-position-long-short')
        player_data['position'] = position_tag.get_text(strip=True) if position_tag else ''

        # Hometown and High School
        hometown_tag = card.select_one('.sidearm-roster-player-hometown')
        if hometown_tag:
            hometown = hometown_tag.get_text(strip=True).split(' / ')
            player_data['hometown'] = hometown[0] if len(hometown) > 0 else ''
            player_data['high_school'] = hometown[1] if len(hometown) > 1 else ''
        else:
            player_data['hometown'] = ''
            player_data['high_school'] = ''

        # Previous School
        player_data['previous_school'] = ''  # Placeholder, as the previous school data isn't present

        # Jersey
        jersey_tag = card.select_one('.sidearm-roster-player-jersey-number')
        player_data['jersey'] = jersey_tag.get_text(strip=True).replace('#', '') if jersey_tag else ''

        # Add the player data to the list
        players.append(player_data)
    return players

def player_card(team, season):
    ...
    players = []
    player_cards = soup.select('.s-person-card__content')
    for card in player_cards:
        # Initialize a dictionary for each player
        player_data = {}
        # Extract player details
        player_data['team_id'] = team['ncaa_id']
        player_data['team'] = team['team']
        player_data['season'] = season

        # Name
        name_tag = card.select_one('.s-person-details__personal-single-line')
        player_data['name'] = name_tag.get_text(strip=True) if name_tag else ''

        # Year
        year_tag = card.select('.s-person-details__bio-stats-item')[1] if len(card.select('.s-person-details__bio-stats-item')) > 1 else None
        player_data['year'] = year_tag.get_text(strip=True).replace('Academic Year', '') if year_tag else ''

        # Height
        height_tag = card.select('.s-person-details__bio-stats-item')[2] if len(card.select('.s-person-details__bio-stats-item')) > 2 else None
        player_data['height'] = height_tag.get_text(strip=True).replace('Height', '') if height_tag else ''

        # Position
        position_tag = card.select('.s-person-details__bio-stats-item')[0] if card.select('.s-person-details__bio-stats-item') else None
        player_data['position'] = position_tag.get_text(strip=True).replace('Position', '') if position_tag else ''

        # Hometown
        hometown_tag = card.select('.s-person-card__content__person__location-item')[0] if card.select('.s-person-card__content__person__location-item') else None
        player_data['hometown'] = hometown_tag.get_text(strip=True).replace('Hometown', '') if hometown_tag else ''

        # High School
        high_school_tag = card.select('.s-person-card__content__person__location-item')[1] if len(card.select('.s-person-card__content__person__location-item')) > 1 else None
        player_data['high_school'] = high_school_tag.get_text(strip=True).replace('Last School', '') if high_school_tag else ''

        # Previous School
        player_data['previous_school'] = ''  # Placeholder, adjust if data is found on the page

        # Jersey
        jersey_tag = card.select_one('.s-stamp__text')
        player_data['jersey'] = jersey_tag.get_text(strip=True).replace('Jersey Number', '') if jersey_tag else ''

        # URL
        link_tag = card.select_one('a')
        player_data['url'] = urljoin(url, link_tag['href']) if link_tag and 'href' in link_tag.attrs else ''

        player_id = urlparse(player_data['url']).path.split('/')[-1]
        player_data['id'] = player_id
...
def shotscraper_card(team, season):
    ...
    new Promise((resolve) => {
        setTimeout(() => {
            resolve(Array.from(document.querySelectorAll('.s-person-card__content'), el => {
            const id = '';
            const name = el.querySelector('.s-person-details__personal-single-line').innerText;
